...
cron 30 * * * * jxnn.js
活动入口：京喜APP-首页-牛牛福利社
已支持IOS双京东账号,Node.js支持N个京东账号   只助力前3个账号
脚本兼容: Node.js
脚本地址：https://ghproxy.com/https://github.com/LubooC/Script/blob/main/jxnn.js
 */
...
$.result = [];
$.shareCodes = [];
let cookiesArr = [], cookie = '', token = '', UA, UAInfo = {}, dwLvl, ddwPaperMoney, date,code='',wclists=[],rwlist = [],taskid='';
...
!(async () => {
  ...
  for (let i = 0; i < cookiesArr.length; i++) {
    ...
    for (let j = 0; j < $.shareCodes.length; j++) {
      console.log(`【${$.UserName}】去助力【${$.shareCodes[j].userName}】邀请码：${$.shareCodes[j].token}`);
      code = $.shareCodes[j].token
      await nnzl()
      await $.wait(1000)
    }
  }
  ...
  try {
    ...
    if(rwlist.length != 0){
      for (let i = 0; i < rwlist.length; i++) {
        ...
        //如果没找到完成的任务
        if(k==0){
          console.log(`任务：`+rwlist[i].taskName);
          taskid = rwlist[i].taskId
          await gotask()
          await $.wait(3000)
        }
...
function go() {
  return new Promise((resolve) => {
    let url = `${JD_API_HOST}pgcenter/task/UserTaskFinish?sceneval=2&taskid=${taskid}&_stk=sceneval%2Ctaskid&_ste=1`;
...
function gotask() {
  return new Promise((resolve) => {
    let url = `${JD_API_HOST}pgcenter/task/drawUserTask?sceneval=2&taskid=${taskid}&_stk=sceneval%2Ctaskid&_ste=1`;
    url += `&h5st=${decrypt(Date.now(), '', '', url)}&_=${Date.now() + 2}&sceneval=2&g_login_type=1&g_ty=ls`;
    let request = {
      url,
      headers: {
...
function nnzl() {
  return new Promise((resolve) => {
    let url = `${JD_API_HOST}pgcenter/sign/helpSign?sceneval=2&token=${code}&flag=1&_stk=flag%2Csceneval%2Ctoken&_ste=1`;
    url += `&h5st=${decrypt(Date.now(), '', '', url)}&_=${Date.now() + 2}&sceneval=2&g_login_type=1&g_ty=ajax`;
    // console.log(url);
    let request = {
      url,
      headers: {
    ...
    $.get(request, (err, resp, data) => {
      try {
        if (err) {
          ...
          if(data.retCode == 0){
            console.log("助力成功🥕");
          }else if(data.retCode == 30009){
            console.log("已经助力过了~");
          }else if(data.retCode == 60009){
            console.log("助力失败，火爆了");
          }else if(data.retCode == 30003){
            console.log("没有登录！❌");
          }else if(data.retCode == 30010){
            console.log("助力已满！🔔");
          }else if(data.retCode == 30014){
            console.log("不能给自己助力");
          }
        }
      } catch (e) {
...
function list() {
  return new Promise((resolve) => {
    let url = `${JD_API_HOST}pgcenter/task/QueryPgTaskCfg?sceneval=2&_stk=sceneval&_ste=1`;
    url += `&h5st=${decrypt(Date.now(), '', '', url)}&_=${Date.now() + 2}&sceneval=2&g_login_type=1&g_ty=ls`;
    let request = {
      url,
      headers: {
...
function wclist() {
  return new Promise((resolve) => {
    let url = `${JD_API_HOST}pgcenter/task/QueryUserTask?sceneval=2&taskType=0&_stk=sceneval%2CtaskType&_ste=1`;
    url += `&h5st=${decrypt(Date.now(), '', '', url)}&_=${Date.now() + 2}&sceneval=2&g_login_type=1&g_ty=ls`;
    let request = {
      url,
      headers: {
...
function helpcode() {
  return new Promise((resolve) => {
    let url = `${JD_API_HOST}pgcenter/sign/UserSignNew?sceneval=2&source=&_stk=sceneval%2Csource&_ste=1`;
  url += `&h5st=${decrypt(Date.now(), '', '', url)}&_=${Date.now() + 2}&sceneval=2&g_login_type=1&g_ty=ajax`;
  let request = {
    url,
    headers: {
    ...
    $.get(request, (err, resp, data) => {
      try {
        if (err) {
          ...
          // console.log(data);
          data = JSON.parse(data);
          console.log(`获取助力码成功：${data.data.token}\n`);
          $.shareCodes.push({
...
function taskUrl(function_path, body) {
  let url = `${JD_API_HOST}pgcenter/${function_path}?strZone=jxbfd&bizCode=jxbfd&source=jxbfd&dwEnv=7&_cfd_t=${Date.now()}&ptag=7155.9.47&${body}&_stk=_cfd_t%2CbizCode%2CdwEnv%2CdwType%2Cptag%2Csource%2CstrZone&_ste=1`;
  url += `&h5st=${decrypt(Date.now(), '', '', url)}&_=${Date.now() + 2}&sceneval=2&g_login_type=1&g_ty=ls`;
  return {
    url,
    headers: {
