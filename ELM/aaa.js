...
!(async () => {
    if (typeof $request !== "undefined") {
        ...
        if (!$.isNode()) {
            ...
            for (let i = 0; i < elmckArr.length; i++) {
                if (elmckArr[i]) {
                    elmck = elmckArr[i];
                    $.index = i + 1;
                    console.log(`\n开始【饿了么账户 ${$.index}】`)
    ...
    return new Promise((resolve) => {
        ...
        $.get(url, async (err, resp, data) => {
            try {
                ...
                if (result.success) {
                    queryTagCode = result.outputJson.moduleList[2].content.$attr.queryTagCode
                    acceptTagCode = result.outputJson.moduleList[2].content.$attr.acceptTagCode
    ...
    return new Promise((resolve) => {
        ...
        $.get(url, async (err, resp, data) => {
            try {
                ...
                let result = JSON.parse(data);
                if (result.success) {
                    console.log(`当前吃货豆：${result.accountInfos[0].count}`);
                }
            } catch (e) {
                $.logErr(e, resp);
            } finally {
...
function supportor() {
    return new Promise((resolve) => {
        let url = {
            ...
            body:`{"ownerId":"bfb0188","fromOfficialAccount":false,"referUserId":"","restaurantId":"","referCode":"","referChannelCode":"","referChannelType":"","fromWeChatApp":false,"bizType":"1","v":"2.9","chInfo":"ch_app_chsub_Kouling","actId":"1","longitude":113.387138366699${num},"latitude":22.9312763214111${num}}`,
            headers: {
             "Cookie": elmck,
...
function running(missionDefId, missionCollectionId, missionType) {
    // console.log(missionDefId, missionCollectionId, missionType);
    return new Promise((resolve) => {
        let url = {
